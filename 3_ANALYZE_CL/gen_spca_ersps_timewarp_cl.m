...
% Chang Liu - 20240212 run SPCA on the ERSP
...
study_fName_1 = 'rest_study_new_all_comps';
...
if ~exist([load_dir filesep study_fName_1 '.study'],'file')|OVERWRITE_study    
    ...
    [STUDY,ALLEEG] = mim_create_epoch_study_ersp([],'epoch_study_new_all_comps',load_dir,['GAIT_EPOCHED_ALL' filesep '0p250p50p751p0flatlowmedhigh'],'GAIT_EPOCHED_ALL','SLIDING_EPOCHED_ALL');
...
for subj_i = 1:length(ALLEEG)
    ...
    icatimf_f = [EEG.filepath filesep sprintf('restERSP_%s.mat',EEG.subject)];
    if ~exist(icatimf_f,'file') || OVERWRITE_ERSP% || any(strcmp(EEG.subject,FINISHED_ADULTS))
...
%                     'trialbase','off','basenorm','on'}); %ERSP
        ...
        restERSP.times = times;
        restERSP.logfreqs = logfreqs;
        restERSP.parameters = parameters;
        restERSP.logersp = logersp;
    
        save([EEG.filepath filesep ['restERSP_',EEG.subject,'.mat']],'restERSP');

    else
        fprintf('Timewarped ERSPs already calculated for resting condition %s \n',EEG.subject);
        load([EEG.filepath filesep ['restERSP_',EEG.subject,'.mat']]);
    end
...
% parfor (subj_i = 1,SLURM_POOL_SIZE)
    ...
    EEG = ALLEEG(subj_i);
    icatimf_f = [EEG.filepath filesep sprintf('gaitERSP_%s.mat',EEG.subject)];
...
%             EEG = pop_selectevent(EEG, 'cond','high','deleteevents','off','deleteepochs','on','invertepochs','off'); 1:size(EEG.icawinv,2)
        ...
        %% subtract baseline
        gaitERSP_subRest = bsxfun(@minus, logersp, mean(restERSP.logersp(:,:,:),2)); %restersp is frequency x time x component
        gaitERSP_subRest = permute(gaitERSP_subRest,[2,3,1]);%required by spca times x component x freq
        GPM = bsxfun(@minus,gaitERSP_subRest,mean(gaitERSP_subRest));
        
        % version - do not subtract rest
        
        %% get the denoise coefficient
        [ERSP_corr, GPM_corr, PSC1, ~,V] = specPCAdenoising(gaitERSP_subRest);

        gaitERSP.times = times;
        gaitERSP.logfreqs = logfreqs;
        gaitERSP.parameters = parameters;
        gaitERSP.logersp = logersp;
        gaitERSP.gaitERSP_subRest = gaitERSP_subRest;
        gaitERSP.ERSP_corr = ERSP_corr;
        gaitERSP.GPM_corr = GPM_corr;
        gaitERSP.PSC1 = PSC1;
        gaitERSP.V = V;
        gaitERSP.GPM = GPM;
        
        save([EEG.filepath filesep ['gaitERSP_',EEG.subject,'.mat']],'gaitERSP');

    else
        fprintf('Timewarped ERSPs already calculated for %s \n',EEG.subject);
        load([EEG.filepath filesep ['gaitERSP_',EEG.subject,'.mat']]);
    end
...
%     save([EEG.filepath filesep 'allersp_gait.mat'],'allersp_gait','alltimes_gait');
    ...
    for ci = 1:length(COND_DESIGNS)
        ...
        try
            ...
            [~, times,logfreqs,parameters,logersp] = std_ersp_cl(tempEEG,'components',1:size(EEG.icawinv,2),'savetrials','off',...
                          'recompute','on','fileout', filebase,...
                          'parallel','on','cycles',ERSP_PARAMS.cycles,...
                          'nfreqs',length((ERSP_PARAMS.freqrange(1):ERSP_PARAMS.freqrange(2))),'ntimesout',TIMEWARP_NTIMES,...
                          'baseline',nan(),'timewarp',timewarp_param,...
                          'timewarpms',timewarpms_param);

            gaitERSP_subRest = bsxfun(@minus, logersp, mean(restERSP.logersp(:,:,:),2));
            gaitERSP_subRest = permute(gaitERSP_subRest,[2,3,1]);%required by spca times x component x freq
            GPM = bsxfun(@minus,gaitERSP_subRest,mean(gaitERSP_subRest));        

            [ERSP_corr, GPM_corr, PSC1] = specPCAdenoising(gaitERSP_subRest, gaitERSP.V);% V matrix:  freqs x freqs 

            gaitERSP_cond.ID         = EEG.subject;
            gaitERSP_cond.cond       = COND_DESIGNS{ci};
            gaitERSP_cond.times = times;
            gaitERSP_cond.logfreqs = logfreqs;
            gaitERSP_cond.logersp    = logersp;
            gaitERSP_cond.gaitERSP_subRest    = gaitERSP_subRest;
            gaitERSP_cond.ERSP_corr  = ERSP_corr;
            gaitERSP_cond.GPM_corr   = GPM_corr;
            gaitERSP_cond.PSC1       = PSC1;
            gaitERSP_cond.GPM        = GPM;

            save([EEG.filepath filesep ['gaitERSP_',EEG.subject,'_',COND_DESIGNS{ci},'.mat']],'gaitERSP_cond');

            %% plot figures
            % ERSP uncorrected
            cfg.f_axis = gaitERSP_cond.logfreqs;
            cfg.t_axis = gaitERSP_cond.times;
            clim = 9;
            f1 = figure();subplot(1,2,1);
            data = squeeze(mean(gaitERSP_subRest,2));
            contourf(cfg.t_axis, cfg.f_axis,data', 50,'linecolor','none'); hold on % plot ERSP

            subplot(1,2,2);
            data = squeeze(mean(ERSP_corr,2));
            contourf(cfg.t_axis, cfg.f_axis,data', 50,'linecolor','none'); hold on % plot ERSP        
            caxis([-clim clim]);
            saveas(f1,[EEG.filepath filesep ['ERSP_',EEG.subject,'_',COND_DESIGNS{ci},'_figure.jpg']])

            f2 = figure();subplot(1,2,1);
            data = squeeze(mean(gaitERSP_cond.GPM,2));
            contourf(cfg.t_axis, cfg.f_axis,data', 50,'linecolor','none'); hold on % plot ERSP
            caxis([-clim clim]);
            colorbar;

            subplot(1,2,2);
            data = squeeze(mean(gaitERSP_cond.GPM_corr,2));
            contourf(cfg.t_axis, cfg.f_axis,data', 50,'linecolor','none'); hold on % plot ERSP        
            colorbar;
            saveas(f2,[EEG.filepath filesep ['GPM_',EEG.subject,'_',COND_DESIGNS{ci},'_figure.jpg']]);

            close all

            comp_test = [12];
            f2 = figure();subplot(1,2,1);
            data1 = squeeze(gaitERSP_cond.GPM(:,comp_test,:));
            contourf(cfg.t_axis, cfg.f_axis,data1', 50,'linecolor','none'); hold on % plot ERSP
    %         caxis([-clim clim]);
            colorbar;

            subplot(1,2,2);
            data2 = squeeze(gaitERSP_cond.GPM_corr(:,comp_test,:));
            contourf(cfg.t_axis, cfg.f_axis,data2', 50,'linecolor','none'); hold on % plot ERSP        
            colorbar;
            saveas(f2,[EEG.filepath filesep ['GPM_',EEG.subject,'_',COND_DESIGNS{ci},'_1IC_figure.jpg']]);
