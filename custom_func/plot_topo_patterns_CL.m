...
function h = plot_topo_patterns_CL(patterns, chanlocs, varargin)
...
addParamValue(ip, 'chan_to_plot',[],@isnumeric);
...
chan_to_plot = ip.Results.chan_to_plot;

weights(weights<1e-10) = 1e-10;

weights_too_low = weights < minweight;

% Added by Chang, plot specific channels regardless of weights criteria
if isempty(chan_to_plot)
    patternNumbersToPlot = find(~weights_too_low);
    weights(weights_too_low)=[];
    patterns(:,weights_too_low)=[];
else
    patternNumbersToPlot = chan_to_plot;
    weights = weights(chan_to_plot);
    patterns = patterns(:,chan_to_plot);
end
