function [] = mim_gen_cluster_figs_CL_statsonly(STUDY,ALLEEG,save_dir,total_cluster,varargin)
...
p0 = 1;p1 = 1;
for i = 3:max(total_cluster) 
    if ismember(i, total_cluster)
...
%color_order = color_order(CLUSTERS_TO_PLOT-2)
...
if DO_GROUP_DIPOLE
    ...
    STUDY.etc.dipparams.centrline = 'on';
    
    std_dipplot_CL(STUDY,ALLEEG,'clusters',CLUSTERS_TO_PLOT,'figure','off','mode','together_averaged_only','spheres','off','projlines','off',...
        'color_CL',color_order,'projlines','on','dipsize',100*ones(size(color_order)));%default=40
    
    fig_i = get(groot,'CurrentFigure');
    set(fig_i,'position',DIP_SING_POS,'color','k')
    camzoom(1);
    % camzoom(1.2^2);    
    view([42,30])
    set(fig_i, 'InvertHardcopy', 'off');
    exportgraphics(fig_i,[save_dir filesep sprintf('dipplot_avgdipspc.jpeg')],'Resolution',300,'BackgroundColor', 'black');
    exportgraphics(fig_i,[save_dir filesep sprintf('dipplot_avgdipspc.pdf')],'ContentType','vector','Resolution',300,'BackgroundColor', 'black');
    saveas(fig_i,[save_dir filesep sprintf('dipplot_avgdipspc.fig')],'fig');
...
%     exportgraphics(fig_i,[save_dir filesep sprintf('dipplot_avgdipspc_sagittal.pdf')],'ContentType','vector','Resolution',300);
    ...
    std_dipplot_CL(STUDY,ALLEEG,'clusters',CLUSTERS_TO_PLOT,'figure','off','mode','together_averaged_multicolor','spheres','off','projlines','off',...
        'color_CL',color_order,'projlines','on');
