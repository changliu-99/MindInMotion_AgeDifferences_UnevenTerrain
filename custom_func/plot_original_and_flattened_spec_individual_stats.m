...
function plot_original_and_flattened_spec_individual_stats(cfg,cls)
    color = cfg.color;
    ...
    for g = 1
        ...
        for k = cls
            brain_area = cfg.brain_area;
                switch brain_area
                    case 'SMA'
                        ylim_intv_orig = [-50 10];
                        ylim_intv_fooof = [-5 30];
                    case 'PP'
                        ylim_intv_orig = [-30 -5];
                        ylim_intv_fooof = [-1 15];
                    case 'Precuneus'
...
%                     color_dark(i,:),color_light(i,:));% input need to be a row vector
                plot(fooof_freq,data,'color',color_light(i,:));
            end
            for i = cond
                data = spec_data_original{g}{k}{i,1}';
...
%                     color_dark(i,:),color_light(i,:));% input need to be a row vector
            ...
            ax = gca;       
            axsignif = highlight_CL(ax, fooof_freq,pcond{k}{1}(:,2), 'background', 'Frequency(Hz)');
            xlim([4 40]);
            plot([0 40],[0 0],'-','color',[0.5 0.5 0.5]);
            ylabel('Flattened PSD');
        %     set(ax,'LineWidth',1)
            set(ax,'FontName','Arial','FontSize',10)
            xline([4],'--'); xline([8],'--'); xline([13],'--'); xline([30],'--');
...
%                     color_dark(i,:),color_light(i,:));% input need to be a row vector
            ...
            ylim(ylim_intv_fooof);
            ax = gca;       
            axsignif = highlight_CL(ax, fooof_freq, fooof_freq,pcond{k}{2}(:,2), 'background', 'Frequency(Hz)');
            xlim([4 40]);
            plot([0 40],[0 0],'-','color',[0.5 0.5 0.5]);
            ylabel('');
        %     set(ax,'LineWidth',1)
