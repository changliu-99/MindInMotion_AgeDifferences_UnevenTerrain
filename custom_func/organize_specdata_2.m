function [specdata_all_2group,specdata_all_3group] = organize_specdata_2(specdata_rest,specdata_gait,STUDY,cluster_num,cond_idx)
    ...
    specdata_all_2group{1,1} = specdata_rest{1}(:,STUDY.cluster(cluster_num).group == 1);
    specdata_all_2group{1,2} = specdata_rest{1}(:,STUDY.cluster(cluster_num).group > 1);
    for i = cond_idx
    ...
    specdata_all_3group{1,1} = specdata_rest{1}(:,STUDY.cluster(cluster_num).group == 1);
    specdata_all_3group{1,2} = specdata_rest{1}(:,STUDY.cluster(cluster_num).group == 2);
    specdata_all_3group{1,3} = specdata_rest{1}(:,STUDY.cluster(cluster_num).group == 3);
    for i = cond_idx
