...
function [outsources, XX, YY, ZZ, XO, YO, ZO] = dipplot_simplified_CL( sourcesori, varargin )
...
if isstr(g.mri)
    try
        ...
        % (02/23/2022, JS: Change made to fieldname get to make it more
        % robust.
        try
            mriFieldName = 'mriOut';
            g.mri = getfield(g.mri,mriFieldName);
        catch
            g.mri = g.mri.mri;
        end
    catch
    ...
    if isempty(g.meshdata)
        ...
        try
            if isstr(g.meshdata)
                ...
                % (02/23/2022), JS: added try/catch to handle different
                % field names from mat files.
                try
                    meshFieldName = 'volOut';
                    tmp.vol = getfield(tmp,meshFieldName);
                catch
                    %
                end
            ...
            disp('Unrecognize model file (probably CTF)')
        end
    end
end
