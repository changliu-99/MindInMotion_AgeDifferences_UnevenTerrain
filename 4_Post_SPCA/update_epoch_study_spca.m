...
dipfit_norm_fPaths_old = cell(1,length([SUBJ_ITERS{:}]));
...
for group_i = 1:length(SUBJ_ITERS)
    ...
    for subj_i = sub_idx
        ...
        if ~isempty(dir([path_clean filesep '*.set'])) & ~isempty(dir([path_epoch filesep '*.set']))
            ...
            tmp_epoch = dir([fPaths_epoch{cnt} filesep '*.set']);
            if ispc
                JS_folder = 'M:\jsalminen\GitHub\par_EEGProcessing\src\_data\MIM_dataset\_studies\11262023_YAOAN104_iccRX0p65_iccREMG0p4_changparams';
            else
                JS_folder = '/blue/dferris/jsalminen/GitHub/par_EEGProcessing/src/_data/MIM_dataset/_studies/11262023_YAOAN104_iccRX0p65_iccREMG0p4_changparams';
            end
            fPaths_dipole_old{cnt} = [JS_folder filesep SUBJ_PICS{group_i}{subj_i} filesep 'clean'];
            fPaths_dipole_new{cnt} = [OUTSIDE_DATA_DIR filesep SUBJ_PICS{group_i}{subj_i} filesep 'clean'];
            try
                ...
                dipfit_norm_fPaths_new{cnt} = [fPaths_dipole_new{cnt} filesep 'dipfit_fem_norm_ants_fixed.mat'];
                %- Prints
        ...
        %         fprintf('DIPFIT Exists: %i\n',exist(dipfit_fPaths{cnt},'file'));
                fprintf('Normalized DIPFIT Exists: %i\n',exist(dipfit_norm_fPaths_old{cnt},'file'));
            catch e
                fprintf('==== Subject %s Paths ====\n',SUBJ_PICS{group_i}{subj_i})
                fprintf('%s\n',getReport(e))
                dipfit_norm_fPaths_old{cnt} = [];
                dipfit_norm_fPaths_new{cnt} = [];
            end
        else 
...
inds = logical(cellfun(@(x) exist(x,'file'),dipfit_norm_fPaths_old));
chanlocs_fPaths = chanlocs_fPaths(inds);
dipfit_norm_fPaths_old = dipfit_norm_fPaths_old(inds);
dipfit_norm_fPaths_new = dipfit_norm_fPaths_new(inds); 
fPaths = fPaths(inds);
fNames = fNames(inds);
...
for subj_i = LOOP_VAR
    if isempty(dir([fPaths_epoch{subj_i} filesep '*_addinfo.set'])) | OVERWRITE_study
        [EEG] = mim_create_alleeg_spca(fNames(subj_i),fPaths(subj_i),subjectNames(subj_i),...
            save_dir,'conditions',conditions(subj_i),'groups',groups(subj_i),...
            'sessions',sessions(subj_i),'dipfit_norm_fPaths',dipfit_norm_fPaths_new{subj_i});
        EEG = reject_eye_ic_nosave(EEG);

        EEG_epoch = pop_loadset('filename',fNames_epoch(subj_i),'filepath',fPaths_epoch{subj_i});
        tmp_name = [EEG_epoch.filename(1:end-4),'_addinfo_old.set'];
        if isempty(dir([fPaths_epoch{subj_i} filesep '*_addinfo_old.set']))
            [EEG_epoch_old] = pop_saveset(EEG_epoch,'filename',tmp_name,'filepath',EEG_epoch.filepath);
        else
            fprintf('addinfo_old.set already exist for %s',subjectNames{subj_i});
        end
        
        EEG_epoch.chanlocs = EEG.chanlocs;
        EEG_epoch.dipfit = EEG.dipfit;
        EEG_epoch.etc.ICArej = EEG.etc.ICArej;

        tmp_name = [EEG_epoch.filename(1:end-4),'_addinfo.set'];
        [EEG_epoch] = pop_saveset(EEG_epoch,'filename',tmp_name,'filepath',EEG_epoch.filepath);
        fprintf('Added dipole info for %s \n',subjectNames{subj_i});
    else
        fprintf('File already exist for %s \n',subjectNames{subj_i});
    end
end                   
