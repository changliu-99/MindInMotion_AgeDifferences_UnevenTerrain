function  [erspDiff_wind] = ...
    runPairwiseCond_rel_flat(allersp,alltimes,allfreqs,config)   
    ...
    title_plot = {'','low - flat','med - flat','high - flat'};
...
%          common_ersp = mean(mean(mean(allersp_nolog(:,baseidx,:,:),3),4),2);
    ...
    f1 = figure('color','white','position',[200 200 700 400],'renderer','Painters');     
    for group = 1:2 %young and old
    ...
    % 
            STUDY.etc.statistics.groupstats = 'off';
            STUDY.etc.statistics.condstats = 'on';
            %             STUDY.design.variable(1) = [];
            curr_ersp_wind = allerspdata_meanSubj{j,group}(freqidx,baseidx,:);%young
            ref_ersp_wind = allerspdata_meanSubj{1,group}(freqidx,baseidx,:);%flat
            %[pcond, pgroup, pinter] = erspStats(STUDY,{curr_ersp;ref_ersp});
            [pcond, pgroup, pinter] = feval(fcn, STUDY,{curr_ersp_wind;ref_ersp_wind});
            [erspDiff_wind{group}(j).raw] = [mean(curr_ersp_wind-ref_ersp_wind,3)];
            [erspDiff_wind{group}(j).masked] = [erspDiff_wind{group}(j).raw.*pcond{1,1}];
            [erspDiff_wind{group}(j).pcond] = pcond{1,1};
                
            subplot(2,length(allerspdata_meanSubj),j+(group-1)*4)
            colormap(colormap_ersp);
            faceAlpha_mask = ones(size(erspDiff_wind{group}(j).pcond))*0.7; %alpha range [0-1] where 0 is fully transparent and 1 = fully opaque
            faceAlpha_mask(erspDiff_wind{group}(j).pcond == 1) = 0; %0 is significant? 1 is not? 
            %     contourf(alltimes3, allfreqs3, erspDiff(4).raw,200,...
            %             'linecolor','none');hold on;
            contourf(alltimes(baseidx), allfreqs(freqidx), erspDiff_wind{group}(j).raw,200,...
                       'linecolor','none');hold on;
            imagesc(alltimes(baseidx), allfreqs(freqidx),erspDiff_wind{group}(j).masked,'AlphaData',faceAlpha_mask);
            %- add vertical line
        ...
        hp4 = get(subplot(2,length(allerspdata_meanSubj),4),'Position');
